...
Существует множество разного софта для общения по TCP, поэтому воспользовавшись, например, утилитой Network Assistant мы можем подключиться к нашему устройству и "пингануть его", а если точнее, можно отправить команду сброса координатора и посмотреть ответ.
